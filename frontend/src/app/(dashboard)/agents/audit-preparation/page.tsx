...
import { useAudits, useTriggerAuditPrepAgent } from "@/hooks/use-api";
...
export default function AuditPreparationPage() {
  ...
  const { data: auditsData, isLoading: auditsLoading } = useAudits(orgId);
  const [selectedAuditId, setSelectedAuditId] = useState<string>("");
  const [currentRunId, setCurrentRunId] = useState<string | null>(null);

  const triggerAgent = useTriggerAuditPrepAgent(orgId);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Audit Preparation Agent</h1>
        <p className="text-muted-foreground">Analyze audit readiness and identify evidence gaps</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Run Audit Preparation</CardTitle>
          <CardDescription>Select an audit to analyze readiness and identify evidence gaps.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleTrigger} disabled={!selectedAuditId || triggerAgent.isPending || polling}>
            {triggerAgent.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Play className="mr-2 h-4 w-4" />}
            Run Audit Preparation
          </Button>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center gap-4">
              ...
              {isComplete && (
                <div className="flex items-center gap-2 text-sm text-green-600">
                  ...
                  {run.output_data && ` — ${(run.output_data as Record<string, unknown>).gaps_count || 0} gaps found, readiness score: ${(run.output_data as Record<string, unknown>).readiness_score || "N/A"}`}
                </div>
              )}
              {isFailed && (
