...
async def create_vendor(org_id: UUID, data: VendorCreate, db: DB, current_user: CurrentUser):
    return await vendor_service.create_vendor(db, org_id, data)


@router.get("/stats", response_model=VendorStatsResponse)
async def get_vendor_stats(org_id: UUID, db: DB, current_user: CurrentUser):
    return await vendor_service.get_vendor_stats(db, org_id)


@router.get("/{vendor_id}", response_model=VendorResponse)
async def get_vendor(org_id: UUID, vendor_id: UUID, db: DB, current_user: CurrentUser):
    return await vendor_service.get_vendor(db, org_id, vendor_id)


@router.patch("/{vendor_id}", response_model=VendorResponse)
async def update_vendor(
    org_id: UUID, vendor_id: UUID, data: VendorUpdate, db: DB, current_user: CurrentUser
):
    return await vendor_service.update_vendor(db, org_id, vendor_id, data)


@router.delete("/{vendor_id}", status_code=204)
async def delete_vendor(org_id: UUID, vendor_id: UUID, db: DB, current_user: CurrentUser):
...
async def create_assessment(
    org_id: UUID, vendor_id: UUID, data: VendorAssessmentCreate, db: DB, current_user: CurrentUser
):
...
async def get_assessments(org_id: UUID, vendor_id: UUID, db: DB, current_user: CurrentUser):
