...
async def create_user(org_id: UUID, data: UserCreate, db: DB, current_user: CurrentUser):
    return await user_service.create_user(db, org_id, data)


@router.get("/{user_id}", response_model=UserResponse)
async def get_user(org_id: UUID, user_id: UUID, db: DB, current_user: CurrentUser):
    return await user_service.get_user(db, org_id, user_id)


@router.patch("/{user_id}", response_model=UserResponse)
async def update_user(
    org_id: UUID, user_id: UUID, data: UserUpdate, db: DB, current_user: CurrentUser
):
    return await user_service.update_user(db, org_id, user_id, data)


@router.delete("/{user_id}", status_code=204)
async def delete_user(org_id: UUID, user_id: UUID, db: DB, current_user: CurrentUser):
