...
async def create_incident(org_id: VerifiedOrgId, data: IncidentCreate, db: DB, current_user: ComplianceUser):
    item = await incident_service.create_incident(db, org_id, data)
    await log_audit(db, current_user, "create", "incident", str(item.id), org_id)
    return item


@router.get("/stats", response_model=IncidentStatsResponse)
async def get_incident_stats(org_id: VerifiedOrgId, db: DB, current_user: AnyInternalUser):
    return await incident_service.get_incident_stats(db, org_id)


@router.get("/{incident_id}", response_model=IncidentResponse)
async def get_incident(org_id: VerifiedOrgId, incident_id: UUID, db: DB, current_user: AnyInternalUser):
...
async def update_incident(
    ...
    item = await incident_service.update_incident(db, org_id, incident_id, data, actor_id=current_user.id)
    await log_audit(db, current_user, "update", "incident", str(incident_id), org_id)
    return item


@router.delete("/{incident_id}", status_code=204)
async def delete_incident(org_id: VerifiedOrgId, incident_id: UUID, db: DB, current_user: ComplianceUser):
    await incident_service.delete_incident(db, org_id, incident_id)
    await log_audit(db, current_user, "delete", "incident", str(incident_id), org_id)
...
async def add_timeline_event(
    ...
    item = await incident_service.add_timeline_event(db, org_id, incident_id, data, actor_id=current_user.id)
    await log_audit(db, current_user, "add_timeline_event", "incident", str(incident_id), org_id)
    return item


@router.get("/{incident_id}/timeline", response_model=list[TimelineEventResponse])
async def get_timeline(org_id: VerifiedOrgId, incident_id: UUID, db: DB, current_user: AnyInternalUser):
